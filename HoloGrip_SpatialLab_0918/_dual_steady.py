...
def run(p):
 ...
 try:
  s=serial.Serial(p,460800,timeout=.08); time.sleep(2.0); s.reset_input_buffer(); t0=time.monotonic();
  while time.monotonic()-t0<5.0:
   b=s.readline()
   if not b: continue
   line=b.decode('utf-8','replace').strip(); f=line.split(',')
   if line.startswith('#STAT'): o['stats'].append(line)
   if len(f)>=10 and f[0]=='D' and f[1] in ('L','R'): o['parsed']+=1;o['hands'].add(f[1])
