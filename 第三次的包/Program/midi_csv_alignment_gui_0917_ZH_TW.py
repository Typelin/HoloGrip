...
FRONTEND_TEMPLATE = PACKAGE_ROOT / "Frontend" / "HoloGrip_MIDI_CSV現場檢查_空白模板_0917_ZH_TW.html"
DEFAULT_CSV_ROOT = PACKAGE_ROOT / "Data" / "第三次Raw"
DEFAULT_MIDI_ROOT = PACKAGE_ROOT / "Data" / "第三次MIDI"
DEFAULT_OUTPUT_ROOT = PACKAGE_ROOT / "Data" / "對齊與異常報告"
DEFAULT_BPM = 110.0
...
def build_frontend_data(
    ...
    return {
        ...
        "rawRows": raw_info["row_count"],
        "mode": "midi_csv",
    }


def build_midi_only_data(midi_path: Path, bpm: float) -> dict[str, Any]:
    parsed = pipeline.parse_midi(midi_path, bpm)
    events = [
        {"id": e.event_id, "mt": e.time_ms, "note": e.note, "vel": e.velocity,
         "zone": e.zone_id, "name": e.zone_name, "hand": "?", "conf": 0}
        for e in parsed["events"]
    ]
    duration = max((float(e["mt"]) for e in events), default=0.0)
    return {
        "events": events, "energy": [], "displayEnergy": [], "duration": duration,
        "bpm": bpm, "offset": 0, "status": "midi_only_review", "source": midi_path.stem,
        "midiSource": midi_path.name, "csvSource": "", "energyBinMs": 10,
        "displayEnergyBinMs": 100, "rawRows": 0, "mode": "midi_only",
    }
...
def default_output_dir() -> Path:
    ...
    return DEFAULT_OUTPUT_ROOT / f"現場MIDI_CSV對齊_0917_ZH_TW_{stamp}"
...
class AlignmentGui:
    ...
    def _build_ui(self) -> None:
        ...
        self.midi_only_button = ttk.Button(actions, text="只有 MIDI：先檢查 MIDI", command=self._start_midi_only)
        self.midi_only_button.pack(side="left", padx=(8, 0))
    ...
    def _choose_file(self, kind: str) -> None:
        if kind == "midi":
            path = filedialog.askopenfilename(initialdir=str(DEFAULT_MIDI_ROOT), filetypes=[("MIDI", "*.mid *.midi"), ("所有檔案", "*.*")])
        else:
            path = filedialog.askopenfilename(initialdir=str(DEFAULT_CSV_ROOT), filetypes=[("CSV", "*.csv"), ("所有檔案", "*.*")])
        if path:
    ...
    def _autofill_latest_files(self) -> None:
        csv_files = sorted(DEFAULT_CSV_ROOT.glob("*.csv"), key=lambda p: p.stat().st_mtime, reverse=True)
        midi_files = sorted(
            [*DEFAULT_MIDI_ROOT.glob("*.mid"), *DEFAULT_MIDI_ROOT.glob("*.midi")],
            key=lambda p: p.stat().st_mtime, reverse=True,
        )
        if csv_files:
            self._set_path("csv", csv_files[0])
        if midi_files:
            self._set_path("midi", midi_files[0])
        if csv_files or midi_files:
            self.status_var.set("已自動選取資料夾內最新檔案；請確認檔名後執行檢查。")

    def _start_midi_only(self) -> None:
        ...
        data = build_midi_only_data(self.midi_path, bpm)
        frontend_path = output_dir / "HoloGrip_MIDI單檔檢查_0917_ZH_TW.html"
        frontend_path.write_text(
            render_frontend_html(FRONTEND_TEMPLATE.read_text(encoding="utf-8"), data, False, int(float(self.window_var.get())), media_defaults=False),
            encoding="utf-8",
        )
        (output_dir / "MIDI單檔檢查資料_0917_ZH_TW.json").write_text(
        ...
        self.result_label.config(text=f"MIDI 單檔檢查：{len(data['events']):,} 筆事件")
        self._write_result(f"已讀取 MIDI：{self.midi_path.name}\n事件：{len(data['events']):,} 筆\n已產生：{frontend_path}\n\n這一步只檢查 MIDI；取得 Raw CSV 後再執行 MIDI → CSV 對齊。")
        self.status_var.set("MIDI 單檔檢查頁已產生並開啟。")
        os.startfile(str(frontend_path))
    ...
    def _start_alignment(self) -> None:
        ...
        def worker() -> None:
            try:
                ...
                report_path = output_dir / "MIDI_CSV對齊報告_0917_ZH_TW.json"
                ...
                frontend_path = output_dir / "HoloGrip_MIDI_CSV對齊現場檢查_0917_ZH_TW.html"
                ...
                (output_dir / "現場MIDI_CSV對齊工作區_0917_ZH_TW.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
...
def self_test() -> None:
    if not FRONTEND_TEMPLATE.is_file():
        raise AssertionError(f"找不到前端模板：{FRONTEND_TEMPLATE}")
    for directory in (DEFAULT_CSV_ROOT, DEFAULT_MIDI_ROOT, DEFAULT_OUTPUT_ROOT):
        directory.mkdir(parents=True, exist_ok=True)
    template = FRONTEND_TEMPLATE.read_text(encoding="utf-8")
    if "</body></html>" not in template:
        raise AssertionError("前端模板不完整")
    if not callable(pipeline.build_outputs):
        raise AssertionError("對齊管線未載入")
    print("midi_csv_alignment_gui 0917 package self-test: OK")
