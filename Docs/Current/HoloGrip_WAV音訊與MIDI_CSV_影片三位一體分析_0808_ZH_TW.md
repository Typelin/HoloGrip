...
### `MIDI → 影片 +14,500 ms` 的來源與限制

`+14,500 ms` 不是由影片畫面自動辨識出的真值，也不是 WAV onset 分析推導出的數值。它是目前用影片中的聲音／演奏起始關係與流音提供的音檔做出的**初始影片對時候選**，先讓人工判讀可以跳到大致位置。

本次 WAV 分析得到的是 `MIDI → WAV +0 ms`，只能說 MIDI 與 WAV 的聲音時間關係接近，不能直接證明 `MIDI → 影片 +14,500 ms`。正式手別標記前，仍應在影片中挑一個畫面清楚、聲音明確的 Crash 或 Snare，人工微調影片偏移到實際棒槌接觸瞬間；因此頁面的影片偏移欄保留可調整，不把 14,500 ms 寫死成真值。
