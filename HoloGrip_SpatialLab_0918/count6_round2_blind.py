...
SESSION=LAB/"count6_round2_sessions"/STAMP
...
class HS:
    def __init__(self,h):
        self.port=""
        self.R0=Rot.from_euler("ZYX",META["source_calibration"][h]["zero"],degrees=True).as_matrix()
...
def connect():
    ...
    try:
        ...
        uiq.put(("status",f"已連線 {', '.join(ps[:2])}。第一輪模型已凍結；第二輪資料不會重訓。"))
...
def start_stage():
    ...
    title_var.set(f"第二輪：{stage_name()}")
...
root=tk.Tk();root.title("HoloGrip｜六位置第二輪盲測｜模型凍結");root.geometry("1050x700");root.attributes("-topmost",True)
...
ttk.Label(root,text="六位置第二輪盲測｜第一輪模型已凍結",style="Title.TLabel",anchor="center").pack(fill="x",pady=(18,6))
...
prompt_var=tk.StringVar(value="這一輪不重新訓練。移到『左上』後按開始。");ttk.Label(root,textvariable=prompt_var,style="Mid.TLabel",anchor="center",justify="center",wraplength=1000).pack(fill="x",pady=8)
