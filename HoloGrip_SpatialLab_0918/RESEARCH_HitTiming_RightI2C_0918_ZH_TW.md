...
## 8. 2026-09-18 23:52 上電姿態重現性追加實驗

Session: `第三次的包/Data/電源循環姿態測試/20260918_235201`

修正版每階段使用 300 個連續有效 frame；B_after 的 69 個 invalid 全部發生在重新插電後前約 3 秒穩定等待期，最後 stage summary 使用的 300 frame 為乾淨段。

主要比較：
- A0 vs A_reboot：完整姿態差 16.14°；重力方向差 6.97°
- B_before vs B_after：完整姿態差 18.40°；重力方向差 11.25°
- A0 vs A_after_Bboot：完整姿態差 14.18°；重力方向差 9.31°
- A→B relative-transform mismatch：18.88°

因此這輪仍不能單獨把跨上電差異歸因於 IMU，因為人工回到 A/B 的物理傾角本身就沒有重現到 5° 內。

但在單一 clean stage 內發現更重要的 heading/fusion 穩定性問題：
- A0：前 50 vs 後 50 frame 完整姿態漂移 8.01°，重力方向只漂 0.68°；Euler yaw 約 28.69° → 20.76°
- A_after_Bboot：完整姿態漂移 7.96°，重力方向只漂 1.18°；yaw 約 39.80° → 31.88°
- A_reboot：完整姿態漂移僅 1.09°，重力 0.52°

=> 固定 2~3 秒後直接做 R0 並不可靠；應等待完整姿態輸出本身穩定，而不是只看 Raw Hz / gravity。

R0 sensitivity（8/12 timing-robust 模型，8/5 真鼓 247 events）：
- baseline 0°：95.14%
- yaw/Z reference error 5° worst：91.50%
- yaw/Z 8° worst：81.38%
- yaw/Z 10° worst：72.06%
- yaw/Z 15° worst：44.13%
- pitch/Y 10° worst：93.12%
- roll/X 10° worst：87.45%

=> 模型对 R0 yaw reference error 高度敏感；约 8~10° 的 heading/R0 偏差已经足以造成明显 cross-session 崩坏。这是当前高优先级根因候选。

第三次现场 Field v2 已改为 stability-gated R0：
- reconnect / fault recovery 先要求 300 连续 valid frame
- R0 等 300-frame continuous clean window
- 前后 50-frame mean rotation drift <=2°
- gravity drift <=1.5°
- 只在满足条件后，用最后 200 frame 建立 R0
- 任一 invalid 在 calibration 中立即清空重收
