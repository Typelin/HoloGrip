...
### `MIDI → 影片 +14,520 ms` 的來源與限制

`+14,520 ms` 是把影片音訊、外部 WAV 與 MIDI 事件分別做 onset 對齊後得到的目前候選，不是只由影片畫面或音譜單獨自動辨識出的真值。

本次影片音訊分析得到 `MIDI → 影片 +14,520 ms`，外部 WAV 分析得到 `MIDI → WAV +0 ms`，因此影片音訊與外部 WAV 的時間差也是 `14,520 ms`。正式手別標記前，仍應在影片中挑一個畫面清楚、聲音明確的 Crash 或 Snare，人工微調影片偏移到實際棒槌接觸瞬間；因此頁面的影片偏移欄保留可調整，不把 `14,520 ms` 寫死成不可更改真值。

第一個 Crash 的交叉檢查：MIDI `4,318.182 ms`；外部 WAV onset 約 `4,320 ms`；影片音訊 onset 約 `18,840 ms`。影片與 MIDI 的誤差約 `1.818 ms`，WAV 與 MIDI 的誤差約 `1.818 ms`。影片音訊在 `2,440 ms` 另有一個最早 onset，但它沒有對應第一筆 MIDI 鼓點，不能直接當成第一個 Crash。
...
依目前影片偏移 `+14,520 ms`，影片 `5:33.085` 約對應 MIDI `318,565 ms`。該區實際 MIDI 事件如下：
...
影片時間 = MIDI 時間 + 影片偏移（目前 +14,520 ms）
...
2. **主顯示區**：由上到下依序是影片、MIDI 鼓點與左右手 CSV 活動度局部圖、播放與複核控制；WAV 聲音控制列放在圖表下方作為第三條證據。
...
播放行為也已區分：按下「開始播放完整事件時間軸」時只在起播瞬間對齊一次 MIDI、CSV、影片與 WAV，之後依播放速度讓媒體自然前進；點擊上一筆、下一筆、事件導覽點或手動時間滑桿時，才重新強制跳到該事件的影片／WAV 時間。完整播放自然結束時會一起停止媒體；手動切換事件也會先退出完整播放。

### 目前介面閱讀順序

1. **資料設定**：先選 MIDI、Raw CSV、演奏影片、WAV 與分析報告，再調整 MIDI → CSV、MIDI → 影片、MIDI → WAV 偏移與採樣窗口。
2. **同步顯示**：影片在最上方；MIDI 色點與 CSV 左／右手活動折線在中間同一個局部時間軸。
3. **播放與複核**：顯示在局部圖下方，包含完整播放、上一筆／下一筆 MIDI、倍速、目前窗口播放、雙手乾淨區與異常區導覽。
...
- 影片與 CSV 的 `+14,520 ms`／`+16,000 ms` 仍是候選偏移，正式標記前要用可辨識擊打再次確認。
- `file://` 頁面受瀏覽器政策限制，無法由自動化工具取得截圖或按鈕操作證據；頁面本身的 JavaScript 已通過語法檢查。

## 0808 完整四方對齊稽核

本次使用影片音訊、外部 WAV、MIDI 與 Raw CSV 重新計算全曲對齊，報告位置：
...
| 關係 | 最佳偏移 | 全曲結果 |
|---|---:|---|
| MIDI → 影片 | +14,520 ms | 1,710 / 1,820 在 ±35 ms 內，93.96% |
| MIDI → WAV | +0 ms | 1,632 / 1,820 在 ±35 ms 內，89.67% |
| MIDI → CSV | +16,000 ms | 全曲活動分數最高候選；局部最佳為 +15,990～16,000 ms |

局部 60 秒搜尋結果顯示：影片偏移由前段 +14,520 ms 到後段約 +14,530 ms，變化約 10 ms；WAV 各段維持 0 ms；CSV 只在 10 ms 解析度內落在 15,990～16,000 ms。目前沒有證據顯示需要做逐段校正或時間拉伸。
...
四者套用各自偏移後，第一個 Crash 誤差約 1.818 ms。影片最早的非 MIDI onset 約 2,440 ms，因此不能拿它當第一個 Crash。

## 0808 播放規則

- 預設播放速度為 1x。
- 完整播放開始時只把影片、WAV 與 CSV 設定到起始位置一次；播放中影片／WAV 自然前進，MIDI 色點與 CSV 折線由連續 S.time 游標同步滾動。
- 點擊播放時間滑桿時，游標停在實際點位；局部圖不再吸附到最近 MIDI 事件。
- 上一筆、下一筆、事件導覽點、跳到目前 MIDI 鼓點，才會執行手動事件跳轉。
- 「播放目前選取 MIDI 片段」以所選 MIDI 事件的 CSV 對齊時間為中心，播放 ±採樣窗口；影片、WAV 與時間軸只掃過這一段，播放結束後停止。
- WAV 仍保留在主顯示區下方作為聲音證據，不搶影片與 CSV 的前台閱讀空間。

## 目前可用性判定

四方資料的時間規律目前足以支援「事件瀏覽、片段核對與初步候選標記」。但影片／WAV onset 只能驗證時間，不會自動產生可靠的左手／右手真值；手別與同一鼓點 20 ms 內的重複事件仍需看影片確認。正式訓練資料應把這些狀態保留為 clean_single、clean_dual_reviewed 或 skip_suspected_duplicate，不要把所有 MIDI 點直接當成已標記真值。
