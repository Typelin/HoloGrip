"""HoloGrip formal frozen-model validation GUI.
...
from validation_capture_0917 import (
...
class Round3ValidationApp(live.LiveHitZoneApp):
    def __init__(self) -> None:
        ...
        self.title("HoloGrip 測試上次模型 · Live + 背景保存 Raw")
        self.geometry("980x820")
        self.session_var = tk.StringVar(value=f"TEST_{datetime.now().strftime('%Y%m%d')}_P_NEW_song01")
        self.path_var = tk.StringVar(value="尚未開始模型測試")
        self.preflight_var = tk.StringVar(value="預檢：尚未執行")
        self._build_validation_controls()
        self._append(f"上次封版模型 SHA-256 OK：{self.model_sha}")
        self._append("測上次模型時，這個畫面會背景保存 Raw；不要同時開 03 收集器。測完關掉後，才進行本次新資料收集。")

    def _build_validation_controls(self) -> None:
        # Keep the familiar Live Model UI.  This panel only adds evidence capture
        # for the OLD frozen model test; it is not the new-training-data collector.
        box = ctk.CTkFrame(self, fg_color="#202235", corner_radius=12)
        box.pack(fill="x", padx=18, pady=(0, 10), before=self.log)

        ctk.CTkLabel(
            box,
            text="① 測試上次模型（這一段不是本次訓練資料收集）",
            font=(live.FONT, 18, "bold"), text_color="#F9E2AF",
        ).pack(anchor="w", padx=16, pady=(12, 4))
        ctk.CTkLabel(
            box,
            text="操作跟原本 Live Model 一樣：連線 → 歸零 → 開始測試 → 打鼓 → 結束測試。背景會同步保存 Raw，之後才能拿同一次 MIDI 算真實成績。",
            font=(live.FONT, 14), text_color=live.MUTED, wraplength=900, justify="left",
        ).pack(anchor="w", padx=16, pady=(0, 10))

        row = ctk.CTkFrame(box, fg_color="transparent")
        row.pack(fill="x", padx=12, pady=(0, 10))
        ctk.CTkLabel(row, text="測試場次", font=(live.FONT, 15, "bold")).pack(side="left", padx=(4, 6))
        self.session_entry = ctk.CTkEntry(row, textvariable=self.session_var, width=225, height=38, font=(live.FONT, 14))
        self.session_entry.pack(side="left", padx=4)
        self.start_session_btn = ctk.CTkButton(
            row, text="開始測試並保存 Raw", width=185, height=42,
            font=(live.FONT, 15, "bold"), fg_color="#A6E3A1", text_color="#111",
            command=self._start_validation_session,
        )
        self.start_session_btn.pack(side="left", padx=6)
        self.stop_session_btn = ctk.CTkButton(
            row, text="結束測試並存檔", width=165, height=42,
            font=(live.FONT, 15, "bold"), fg_color="#F38BA8", text_color="#111",
            command=self._stop_validation_session,
        )
        self.stop_session_btn.pack(side="left", padx=6)

        ctk.CTkLabel(
            box, textvariable=self.path_var, text_color=live.MUTED,
            font=(live.FONT, 12), wraplength=900, justify="left",
        ).pack(anchor="w", padx=16, pady=(0, 10))

    def _start_validation_session(self) -> None:
        if not self.connected:
            messagebox.showwarning("尚未連線", "請先像原本 Live Model 一樣連接兩隻手套。")
            return
        if self.recognizer.calibrated_hands != {"L", "R"}:
            messagebox.showwarning("尚未歸零", "請先回固定原點按『歸零』，再開始模型測試。")
            return
        if self.recorder.active:
            messagebox.showinfo("測試已在記錄", str(self.recorder.session_dir))
            return
        path = self.recorder.start(self.session_var.get(), self.model_path, self.model_sha)
        self.phase = "song"
        self.session_calibrated = True
        self.recorder.marker("SONG_START", "上次 Frozen Model 測試開始；MIDI 不進推論")
        self.path_var.set(f"正在測上次模型；Raw 同步保存：{path}")
        self.session_entry.configure(state="disabled")
        self.start_session_btn.configure(state="disabled")
        self._append(f"上次模型測試開始：{path}")
        self._append("這一段只是在考舊模型；不是本次要回去標記/訓練的新資料收集。")
        self._append("Raw 100 Hz 與 Live predictions 同步留存；MIDI 之後才拿來對答案。")
        self.status_var.set("測試進行中。照原本方式打；結束後按『結束測試並存檔』。")

    def _stop_validation_session(self) -> None:
        if not self.recorder.active: return
        session_dir = self.recorder.session_dir
        if self.phase == "song":
            self.recorder.marker("SONG_END", "上次 Frozen Model 測試結束")
        self.recorder.stop()
        ...
        self.path_var.set(f"上次模型測試已保存：{session_dir}")
        self.status_var.set("上次模型測試已保存。拿到同一次 MIDI 後開 02 算成績；接著可關閉本程式，再開 03 收本次新資料。")
    ...
    def _summary_md(summary: dict[str, Any]) -> str:
        lines = [
            "# HoloGrip 正式 Frozen Model 驗證場次摘要", "",
    ...
    def _calibrate(self) -> None:
        ...
        if self.recognizer.calibrated_hands == {"L", "R"} and self.recognizer.generation > before_gen:
            if self.recorder.active:
                ...
                self.session_calibrated = True
            self.preflight_var.set("歸零完成")
            self._append("測試中的重新歸零已留下 CALIBRATE marker。")
    ...
    def _set_phase(self, phase: str, marker: str, note: str) -> None:
        ...
        if marker == "SLOW_TEST_START":
            ...
            self.status_var.set("正式驗證進行中：只有 Raw IMU 進推論；不要改模型、不要改偵測器參數。")
